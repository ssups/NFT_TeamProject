        ...
        {title === '판매 상품으로 등록하기' && (
          <>
            ...
            <p className="text-center text-light">
              판매 완료 시 정산금 : {incomeAfterFee ? incomeAfterFee + ' ether' : '0 ether'}
            </p>
            <p className="text-center text-light">
              판매 완료 시 발생하는 수수료 :{' '}
              {transactionFee ? transactionFee + ' ether' : '0 ether'}
            </p>
        ...
        {title === '경매 상품으로 등록하기' && (
          <>
            <div className="input_item mb-4">
              ...
              <input
                type="number"
                placeholder="최소 단위 0.001"
                ref={_transactionPrice}
                onChange={changeAmountFn}
              />

              <p className="text-center text-light">
                경매 진행 시간을 1 ~ 30 분 중에서 선택해주세요.
              </p>
              <select ref={_endTime}>
                {Array.from({ length: 30 }, (value, index) => index + 1).map(minute => (
            ...
            <p className="text-center text-light">
              경매 낙찰 시 정산금 : {incomeAfterFee ? incomeAfterFee + ' ether' : '0 ether'}
            </p>
            <p className="text-center text-light">
              경매 낙찰 시 발생하는 수수료 :{' '}
              {transactionFee ? transactionFee + ' ether' : '0 ether'}
